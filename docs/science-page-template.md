...
| 2   | **Harvest the live translations** of the sections being replaced (overview, evidence, findings cards, usage, evidence table, references) into a phrase table, so approved translations are reused rather than rewritten.                      | `scripts/harvest-translations.py <template> <out.json>` (Translate & Adapt, read-only)                                                                                                                                    | Phrase table seeded                               |
...
| 10  | Audit.                                                                                                                                                                                                                                        | `scripts/page-audit.py /pages/<page>`, then the central auditor (skill `seo-aiso-validator`; `seo-toolkit/scripts/audit_page.py`). `scripts/aiso-audit-page.py` was deleted 2026-09-24, and its scores are not comparable | External ≥ 9.0; GEO 96 is the accepted table cost |
...
### 6.2 Learned on the copper rollout (2026-09-25 and 26)

- **A key figure is a result, never a date.** Copper's third figure was "2026 · Independent systematic review": a publication year set as a statistic. The
  critic scored it the page's worst finding (it will date itself, reads as a copyright line, and filled the slot where no independent effect size exists).
  The reference pages use the third slot for time to result (Argireline "4 weeks", PDRN "Day 14"); copper now does the same ("4 weeks · First measured
  change", register §8.4). If the evidence has no third number, use a time or a tolerability count from the register, and put independence in a sentence.
- **Index rows are one line each:** claim about 65 characters or fewer, qualifier about 95 or fewer. Copper's 89–97-character claims made 127–151 px rows
  against Argireline's 101 px; cut to length they measure 101 px. Move comparators into the qualifier.
- **Do not copy another page's layout fixes blindly.** PDRN's usage CSS stacks the two charts because its charts differ by 407 px in height. Copper copied
  that rule although its charts differ by 98 px, and the band grew to 1,742 px (Argireline's side by side: 914 px). Measure the two charts side by side first.
- **No `hyphens:auto`** in the overview or index CSS. The reference has none, and it split words inside the bold accent claims on phones.
- **Hero contrast is measured at the worst pixel under each line on the phone image too.** The desktop banner passed at 14:1 while the phone crop put the
  subtitle over bright glass at 3.4:1. The existing overlay setting fixed it (50 → 60, worst 4.57:1), per "standard sections before custom code".
- **The charts reflow at 200% zoom** since 2026-09-26 (`scripts/hub_charts.py`, `tests/test_hub_charts.py`): at 195 px the value column had left the bars
  0 px wide and the table 45 px off-screen. Every page picks this up the next time its charts spec is applied (PDRN, Argireline, Matrixyl and glutathione
  still carry the old CSS).
- **The CTA must name the real product**, in each language as that locale's product title (copper said "Skin Renewal and Repair Serum", a product that does
  not exist).
