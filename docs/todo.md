...
- ⏳ **Named author on the hubs.** The reviewer is now credited (entry above). The author is still the organisation ("By Skingenetix"); the external auditors reward a named individual author with credentials. Malcolm's call whether to name one (e.g. himself as founder, as on Hairgenetix).
...
- ⚠️ **Dutch serves a stale references block on PDRN, Argireline® and copper** (found 2026-09-22). The nl translation of `references.html` is outdated but still served: it has no JSON-LD, and on copper it is missing 2 of the 9 references. Fixed by the retrofit, which registers a fresh translated references block for all five locales.
