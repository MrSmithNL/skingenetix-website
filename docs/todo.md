...
- 🔄 **Evidence Library + study pages — BUILD (verdict reversed 2026-09-22).** `docs/research-2026-study-hubs-credibility.md`: an Evidence Library, plus one appraisal page per qualifying human trial or systematic review (~12–18, null results included), as a `study` metaobject at `/pages/study/<handle>` with `Article` + `isBasedOn` schema, shipped in all six languages at once (so no noindex is needed). Pilot 2 first, then a wave of 5 with an 8-week gate. Needs Malcolm's go-ahead on scope.
