...
- ✅ **Tooling for the rollout** (2026-09-24):
  ...
  - Tests 26 → 38.
- ⏭ **Move PDRN, copper, Matrixyl 3000 and glutathione onto the template**, one page per go-ahead (procedure: template doc §6; what each page has today: §7). Suggested order: PDRN first, as 68% of the qualified search opportunity.
- ⏭ **Argireline loose ends** (template doc §9), to do at the next edit of `evidence_sources`, through `hub-i18n.py`:
  - dead CSS for the old media-with-text overview;
  - the JSON-LD cites 8 works where the table has 10.
